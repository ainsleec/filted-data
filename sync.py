...
ALLOWED_SIGHTING_STATUSES = {ACTIVE_STATUS, SOLD_STATUS}
...
def main():
    ...
    def get_garment_designer(fields):
        return str(fields.get("Designer", "")).strip()
    ...
    print(f"  {len(active_sightings)} Active | {len(sold_sightings)} Sold | "
          f"{len(expired_sightings)} Expired sightings\n")

    # Step 4: Priority garments
    sighting_garment_ids = set()
    for r in active_sightings + sold_sightings:
    ...
    print(f"{'=' * 64}")
    print(f"  Campaigns: {len(priority_campaigns)} | Garments: {len(garments_to_sync)} | "
          f"Sightings: {len(sightings_to_sync)}")
    print(f"{'=' * 64}\n")

    # Step 7: Clean up sold/expired sightings
    print("  Cleaning up sold/expired sightings...")
    ...
    for record in sold_sightings + expired_sightings:
        ...
        if existing_webflow_id:
            ...
            skipped += 1
        time.sleep(0.15)
    print(f"  Sold/Expired: {deleted} deleted | {skipped} already clean | {errors} errors\n")
    ...
    # Step 10: Rebuild garment lookup and sync sightings
